...
class CategoryDisplaySettingsStore(
    ...
    suspend fun getShowDiacritics(categoryId: Long): Boolean = withContext(Dispatchers.IO) {
        val value = database.categoryDisplaySettingsQueries
            .selectShowDiacritics(categoryId)
            .executeAsOneOrNull()
        (value ?: 1L) != 0L
    }

    suspend fun setShowDiacritics(categoryId: Long, enabled: Boolean) = withContext(Dispatchers.IO) {
    ...
    suspend fun toggleShowDiacritics(categoryId: Long): Boolean = withContext(Dispatchers.IO) {
        val current = database.categoryDisplaySettingsQueries
            .selectShowDiacritics(categoryId)
            .executeAsOneOrNull()
